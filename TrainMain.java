...
public class TrainMain {
	...
	try{
	    ...
	    sql = "CREATE TABLE IF NOT EXISTS TRAINDETAILS(TRAIN_NO INTEGER PRIMARY KEY NOT NULL ,NO_OF_TICKETS INTEGER NOT NULL , TRAIN_NAME TEXT NOT NULL, SOURCE_STATION TEXT NOT NULL, DESTINATION_STATION TEXT NOT NULL, DEPARTURE_TIME TIME NOT NULL, ARRIVAL_TIME TIME NOT NULL);";
            stmt.executeUpdate(sql);
	    sql = "CREATE TABLE IF NOT EXISTS ROUTEDETAILS(TRAIN_NO INTEGER NOT NULL ,STATION_NAME TEXT NOT NULL, ARRIVAL_TIME TIME NOT NULL, DEPARTURE_TIME TIME NOT NULL,CONSTRAINT FK FOREIGN KEY(TRAIN_NO) REFERENCES TRAINDETAILS(TRAIN_NO) ON DELETE CASCADE);";
            stmt.executeUpdate(sql);
	    sql = "CREATE TABLE IF NOT EXISTS USERTICKETDETAILS(USERNAME TEXT, TRAIN_NO INTEGER NOT NULL ,FROM_STATION TEXT NOT NULL, TO_STATION TEXT NOT NULL, NO_OF_TICKETS INTEGER NOT NULL, TIME_OF_BOOKING TIMESTAMP, PRIMARY KEY(USERNAME,TRAIN_NO),CONSTRAINT FK FOREIGN KEY(TRAIN_NO) REFERENCES TRAINDETAILS(TRAIN_NO) ON DELETE CASCADE,CONSTRAINT FK1 FOREIGN KEY(USERNAME) REFERENCES USERLOGIN(USERNAME) ON DELETE CASCADE);";
            stmt.executeUpdate(sql);
	    sql = "CREATE TABLE IF NOT EXISTS TICKETSPLITUPS(ID INTEGER, TNO INTEGER NOT NULL ,FROMSTATION TEXT NOT NULL, TOSTATION TEXT NOT NULL, TICKETSAVL INTEGER NOT NULL, MIDDLESTATION TEXT ,PRIMARY KEY(ID), FOREIGN KEY(TNO) REFERENCES TRAINDETAILS(TRAIN_NO) ON DELETE CASCADE);";
            stmt.executeUpdate(sql);
            stmt.close();
	    con.close();
	}catch(Exception e){
